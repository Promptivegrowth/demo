    ...
    return (
        <header className="h-20 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 relative z-10 shadow-sm">
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                        <span>Módulo Retail</span>
                        <ChevronRight className="w-3 h-3" />
                        <span className="text-emerald-500 font-black">PROMPTIVE</span>
                    </div>
            ...
            <div className="flex items-center gap-3">
                {/* Search */}
                <div className="hidden lg:flex items-center bg-slate-100 rounded-2xl px-4 py-2 border border-slate-200/50 w-64 group focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:bg-white transition-all">
                    <Search className="w-4 h-4 text-slate-400 group-focus-within:text-emerald-500" />
                    <input
                        type="text"
                        placeholder="Buscar producto o venta..."
                ...
                <div className="flex items-center gap-1">
                    <button
                        onClick={() => toast.info('Panel de Notificaciones', { description: 'No hay alertas de stock bajo por ahora.' })}
                    ...
                    <button
                        onClick={() => toast.info('Configuración Retail', { description: 'Personaliza tus tickets y comprobantes aquí.' })}
                        className="p-2.5 hover:bg-slate-100 rounded-xl transition-all text-slate-500 hover:text-slate-900"
                    >
                        <Settings2 className="w-5 h-5" />
                    </button>
                    <button
                        onClick={() => toast('Guía de Usuario', { description: 'Soporte especializado para el punto de venta.' })}
                        className="p-2.5 hover:bg-slate-100 rounded-xl transition-all text-slate-500 hover:text-slate-900"
                    >
