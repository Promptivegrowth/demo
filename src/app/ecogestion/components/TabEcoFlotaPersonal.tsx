...
export default function TabEcoFlotaPersonal({ showToast, ecoQuery }: any) {
    ...
    // Formulario modal y Detalle
    const [modal, setModal] = useState<'flota' | 'personal' | 'detalle_flota' | 'detalle_personal' | null>(null)
    ...
    const cargar = async () => {
        ...
        let fArr = Array.isArray(resFlota) ? resFlota : []
        let pArr = Array.isArray(resPersonal) ? resPersonal : []

        if (fArr.length === 0) {
            fArr = [
                { id: 'f1', placa: 'V7X-890', marca: 'Hino', modelo: 'Dutro 300', capacidad_kg: 5500, tipo: 'furgon', estado: 'activo', km: 12500, soat_vto: '2024-12-15', ultimo_mant: '2024-01-20' },
                { id: 'f2', placa: 'B4L-501', marca: 'Isuzu', modelo: 'NLR Comfort', capacidad_kg: 3500, tipo: 'compactador', estado: 'mantenimiento', km: 45200, soat_vto: '2024-10-05', ultimo_mant: '2024-03-10' },
                { id: 'f3', placa: 'F1W-442', marca: 'Volvo', modelo: 'VM 270', capacidad_kg: 12000, tipo: 'cisterna', estado: 'activo', km: 8900, soat_vto: '2025-02-28', ultimo_mant: '2024-02-15' },
            ]
        }
        if (pArr.length === 0) {
            pArr = [
                { id: 'p1', nombres: 'ROBERTO CARLOS LUNA', dni: '45889231', cargo: 'conductor', licencia_conducir: 'Q45889231-AIIIc', estado: 'activo', fecha_ingreso: '2022-05-15', especialidad: 'Materiales Peligrosos' },
                { id: 'p2', nombres: 'MARCO ANTONIO SOLIS', dni: '09223145', cargo: 'ayudante', licencia_conducir: null, estado: 'activo', fecha_ingreso: '2023-01-10', especialidad: 'Gestión de Sólidos' },
                { id: 'p3', nombres: 'ELENA GOMEZ PRADA', dni: '71223409', cargo: 'supervisor', licencia_conducir: 'E71223409-AI', estado: 'activo', fecha_ingreso: '2021-11-20', especialidad: 'Seguridad Industrial' },
            ]
        }
    ...
    const abrirDetalleFlota = (f: any) => { setDetalleData(f); setModal('detalle_flota') }
    const abrirDetallePersonal = (p: any) => { setDetalleData(p); setModal('detalle_personal') }
    ...
    const DetalleFlotaModal = () => (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            ...
            >
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <Truck className="w-5 h-5 text-indigo-500" /> Hoja de Vida - Unidad {detalleData?.placa}
                    </h3>
                    <button onClick={() => setModal(null)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2 bg-slate-900 p-6 rounded-2xl text-white relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-4 opacity-10"><Truck className="w-24 h-24" /></div>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Placa de Rodaje</p>
                            <p className="text-4xl font-black font-mono mt-1 tracking-tighter">{detalleData?.placa}</p>
                            <div className="mt-4 flex items-center gap-3">
                                {ecoBadge(detalleData?.estado)}
                                <span className="text-xs font-bold text-slate-400">Tipo: {detalleData?.tipo?.toUpperCase()}</span>
                            </div>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Marca / Modelo</p>
                            <p className="text-sm font-bold text-slate-800">{detalleData?.marca} {detalleData?.modelo}</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Capacidad Carga</p>
                            <p className="text-sm font-bold text-slate-800">{detalleData?.capacidad_kg} KG</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Kilometraje Act.</p>
                            <p className="text-sm font-bold text-indigo-600 font-mono">{detalleData?.km || '0'} KM</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">SOAT Vencimiento</p>
                            <p className="text-sm font-bold text-rose-500">{detalleData?.soat_vto || 'N/A'}</p>
                        </div>
                        <div className="col-span-2 p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-bold text-emerald-600 uppercase">Último Mantenimiento</p>
                                <p className="text-sm font-bold text-emerald-800">{detalleData?.ultimo_mant || 'No Registra'}</p>
                            </div>
    ...
    const DetallePersonalModal = () => (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            ...
            >
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <Users className="w-5 h-5 text-indigo-500" /> Ficha de Personal - {detalleData?.nombres?.split(' ')[0]}
                    </h3>
                    <button onClick={() => setModal(null)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                    <div className="flex flex-col items-center mb-6">
                        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-4xl text-white font-black shadow-xl mb-3">
                            {detalleData?.nombres?.charAt(0)}
                        </div>
                        <h4 className="text-xl font-bold text-slate-800">{detalleData?.nombres}</h4>
                        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">{detalleData?.cargo}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">DNI Documento</p>
                            <p className="text-sm font-bold text-slate-800 font-mono tracking-widest">{detalleData?.dni}</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Licencia MTC</p>
                            <p className="text-sm font-bold text-amber-600 font-mono">{detalleData?.licencia_conducir || 'S/L'}</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Fecha de Ingreso</p>
                            <p className="text-sm font-bold text-slate-800">{detalleData?.fecha_ingreso || 'No Registra'}</p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Especialidad</p>
                            <p className="text-sm font-bold text-indigo-600">{detalleData?.especialidad || 'General'}</p>
                        </div>
                        <div className="col-span-2 p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <BadgeCheck className="w-6 h-6 text-emerald-500" />
                                <div>
                                    <p className="text-[10px] font-bold text-emerald-600 uppercase">Estado Contractual</p>
                                    <p className="text-sm font-bold text-emerald-800">Activo y Certificado</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <AnimatePresence>
                {(modal === 'flota' || modal === 'personal') && <FormModal />}
                {modal === 'detalle_flota' && detalleData && <DetalleFlotaModal key="det_flota" />}
                {modal === 'detalle_personal' && detalleData && <DetallePersonalModal key="det_pers" />}
            </AnimatePresence>
                                        ...
                                        <tr key={p.id} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex flex-col items-end gap-2">
                                                    ...
                                                    <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            onClick={() => abrirDetallePersonal(p)}
                                                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-teal-50 text-teal-600 hover:bg-teal-100 transition-colors"
